...
import { type Env, workerHandler } from '~/index';
...
describe('v8 Artifacts API', () => {
  ...
  describe('GET artifact endpoint', () => {
    ...
    test('should use the default team ID when none is provided', async () => {
      let request = createArtifactGetRequest(
        `http://localhost/v8/artifacts/${artifactId}`
      );
      ...
      request = createArtifactGetRequest(
        `http://localhost/v8/artifacts/${artifactId}`
      );
  ...
  describe('PUT artifact endpoint', () => {
    ...
    test('should use the default team ID when none is provided', async () => {
      ...
      const artifactStream =
        await workerEnv.STORAGE_MANAGER.getActiveStorage().read(
          `${DEFAULT_TEAM_ID}/${artifactId}`
        );
      const artifact = await StorageManager.readableStreamToText(
        artifactStream!
      );
    ...
    test('should save artifact tag when provided', async () => {
      ...
      const artifactWithMeta =
        await workerEnv.STORAGE_MANAGER.getActiveStorage().readWithMetadata(
          `${teamId}/${artifactId}`
        );
      const artifact = await StorageManager.readableStreamToText(
        artifactWithMeta.data!
      );
      expect(artifact).toEqual(artifactContent);
      expect(artifactWithMeta?.metadata?.customMetadata?.artifactTag).toEqual(
        artifactTag
      );
    });
  ...
  describe('HEAD artifact endpoint', () => {
    ...
    test('should use the default team ID when none is provided', async () => {
      let request = createArtifactHeadRequest(
        `http://localhost/v8/artifacts/${artifactId}`
      );
      let res = await app.fetch(request, workerEnv, ctx);
      expect(res.status).toBe(404);
      ...
      request = createArtifactHeadRequest(
        `http://localhost/v8/artifacts/${artifactId}`
      );
      res = await app.fetch(request, workerEnv, ctx);
      expect(res.status).toBe(200);
