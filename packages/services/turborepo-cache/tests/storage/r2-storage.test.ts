...
describe('r2-storage', () => {
  ...
  describe('listWithMetadata()', () => {
    ...
    test('returns preset keys with metadata', async () => {
      ...
      expect(
        result.keys.every(
          (k) => k.metadata?.staticMetadata.createdAt instanceof Date
        )
      ).toBe(true);
  ...
  describe('readWithMetadata()', () => {
    ...
    test('returns preset value with metadata', async () => {
      ...
      const dataAsText = await StorageManager.readableStreamToText(
        result.data!
      );
      expect(dataAsText).toBe('value1');
      expect(result.metadata?.customMetadata).toEqual(customMetadata);
      expect(result.metadata?.staticMetadata.createdAt instanceof Date).toBe(
        true
      );
    });
  ...
  describe('write()', () => {
    ...
    test('can write ArrayBuffer value', async () => {
      ...
      const result = await storage.read('key1');
      const dataAsBuffer = (await StorageManager.readableStreamToBuffer(
        result!
      )) as ArrayBuffer;
